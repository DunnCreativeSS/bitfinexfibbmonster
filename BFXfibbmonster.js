...
string = "IOTA" + string;		
	...
	if (string.slice(-4) == "USDT"){
		...
		var totaltotal = thetotalbtc;
		console.log(totaltotal);
		totaltotal += thetotalusdt /  btcusd;
		console.log(totaltotal);
		
		totaltotal += thetotaleth * ethbtc;
		console.log(totaltotal);
		...
		+ 'hours: ' + hours + '<br>'
		+ '<h1>PL: ' + (PL) + '%</h1>'
		+ '<h1>BTCUSD: ' + -1 * (1-(btcusd/btcusdold).toFixed(4)) * 100 + '%</h1>'
...
+ '<script>$(document).ready(function(){console.log("ready!");setInterval(function(){$.getJSON("https://bitfinexmarginfibbmonster.herokuapp.com/update",function(o){console.log(o),console.log((new Date).getTime());var a=(new Date).getTime();var pl = []; var btcusd = [];for (var d in jsondata){pl.push([jsondata[d].t, jsondata[d].pl]);btcusd.push([jsondata[d].t, jsondata[d].btcusd])}chart.series[0].setData(pl),chart.series[1].setData(btcusd),chart = Highcharts.stockChart("container", options);})},3e4)});</script>'
...
+ '<script>var prices=JSON.parse($("#prices").text()),btcusd=[],pl=[];for(var p in prices){var t=prices[p].t,btcusdt=prices[p].btcusd,plt=prices[p].pl;btcusd.push([t,btcusdt]),pl.push([t,plt])}var options={title:{text:"Bitfinex Margin Fib Monster"},rangeSelector:{buttons:[{count:1,type:"minute",text:"1m"},{count:5,type:"minute",text:"5m"},{count:10,type:"minute",text:"10m"},{count:30,type:"minute",text:"30m"},{count:1,type:"hour",text:"1h"},{type:"all",text:"All"}],inputEnabled:!1,selected:2},yAxis:[{labels:{align:"right",x:-3},title:{text:"%"},height:"100%",lineWidth:2,resize:{enabled:!0}}],tooltip:{split:!0},plotOptions:{series:{dataGrouping:{enabled:!1}}},series:[{cropThreshold:999900,name:"PL",data:pl,color:"green"},{cropThreshold:999900,name:"btcusd",data:btcusd,color:"black"}]};</script>'
 + '<script>var chart = Highcharts.stockChart("container", options);</script>');
...
var collection = dbo.collection(process.env.prices2);
				...
				'prices': {t: d, btcusd: -1 * (1-(btcusd/btcusdold)) * 100, pl: PL}
			}, function(err, res) {
				if (err) {}
