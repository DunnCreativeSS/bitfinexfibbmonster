...
+ '<script>$(document).ready(function(){console.log("ready!");setInterval(function(){$.getJSON("https://bitfinexmarginfibbmonster.herokuapp.com/update",function(o){console.log(o),console.log((new Date).getTime());var a=(new Date).getTime();for (var d in jsondata){pl.push([jsondata[d].t, jsondata[d].pl]);btcusd.push([jsondata[d].t, jsondata[d].btcusd])}chart.series[0].setData(pl),chart.series[1].setData(btcusd),chart = Highcharts.stockChart("container", options);})},3e4)});</script>'
